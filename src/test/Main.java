...
public class Main
{
	public static void main(String[] args) {
		...
		int teklif = 4;
        Player[] p =new Player[] {new Player(1),new Player(2),new Player(3),new Player(4)};
        Player[] players=null;
        ...
        out:
        do{
        for(int playerNo=k%4;playerNo<(k%4)+4;playerNo++)
        {
			if(Player.count==0) break out;
            if(players[playerNo%4].teklifYapıldımı)
            {
                ...
                if(!s.equals("pas"))
                {
                    ...
                    players[playerNo%4].teklifYapıldımı = false;
                    Player.count--;
                }
        ...
        for(Player player:players)
            System.out.println(player+": "+player.getScore()+" total point: "+player.totalPoint);
        for(Player player:players){
